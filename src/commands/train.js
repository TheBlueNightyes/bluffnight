...
export default async (interaction) => {
    ...
    const msg1 = await interaction.reply(`train sent to ${location}!\n> arriving <t:${currentDate}:R>`)
    const msg2 = await interaction.channel.send(`${trainStuff}`)
    ...
    setInterval(async () => {
        current++
        if (current >= num) {
            await msg1.edit(`train arrived at ${location}!`, true)
            done = true
        }
    }, 1000)
}